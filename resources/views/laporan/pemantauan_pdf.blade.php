...
<head>
    <title>Laporan Pemantauan Bahan Baku</title>
    <style>
        table {
            width: 100%;
...
<body>
    <h2>Laporan Pemantauan Bahan Baku</h2>
    <img src="{{ asset('images/logo_perusahaan.png') }}" class="logo" alt="Logo Perusahaan">
    <div class="header" style="justify-content:center;">
        <h2>Merra Coffee & Talk</h2>
        <h4>Periode: {{ \Carbon\Carbon::parse(request('dari_tanggal'))->format('d M Y') }} -
            {{ \Carbon\Carbon::parse(request('sampai_tanggal'))->format('d M Y') }}</h4>
    </div>
